...
bool SparseMatmulCCSMPI::ValidationImpl() {
  if (GetInput().matrix_a.cols != GetInput().matrix_b.rows) {
    return false;
  }
  return true;
}
    ...
    for (int dest = 0; dest < size; dest++) {
      ...
      if (dest == 0) {
        ...
        for (int i = 0; i <= local_b.cols; i++) {
          local_b.col_pointers[i] = b.col_pointers[dest_start + i] - start_idx;
        }
      } else {
...
void GatherResults(int rank, int size, const SparseMatrixCCS &local_res, SparseMatrixCCS &final_res, int res_rows,
  ...
  if (rank == 0) {
    ...
    std::vector<SparseMatrixCCS> all_locals(size);
    all_locals[0] = local_res;
    ...
    for (int src = 1; src < size; src++) {
      ...
      all_locals[src].cols = recv_cols;
      all_locals[src].col_pointers.resize(recv_cols + 1);

      if (recv_vals_size > 0) {
        all_locals[src].values.resize(recv_vals_size);
        all_locals[src].row_indices.resize(recv_rows_size);
        MPI_Recv(all_locals[src].values.data(), recv_vals_size, MPI_DOUBLE, src, 7, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(all_locals[src].row_indices.data(), recv_rows_size, MPI_INT, src, 8, MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE);
      }
      MPI_Recv(all_locals[src].col_pointers.data(), recv_cols + 1, MPI_INT, src, 9, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
