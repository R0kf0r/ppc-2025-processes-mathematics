...
MPI_Gatherv собирает все части результата обратно на процесс 0. Если процессов больше одного, результат дополнительно рассылается всем через MPI_Bcast (это нужно для тестов).
...
- Broadcast размеров и матрицы B: процесс 0 → все
- Scatterv строк A: процесс 0 → каждому
- Gatherv результатов: все → процесс 0
- Broadcast результата: процесс 0 → все (если процессов > 1)
...
Класс `MatrixMultTaskMPI` с аналогичной структурой. В `RunImpl` реализована параллельная логика с MPI операциями.
...
- Последовательных участков (broadcast, gather)
